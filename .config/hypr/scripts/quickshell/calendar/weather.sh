...
get_data() {
    # ---------------------------------------------------------
    # ZERO-KEY FALLBACK (Open-Meteo free API)
    # ---------------------------------------------------------
    if [[ -z "$KEY" || "$KEY" == "Skipped" || "$KEY" == "OPENWEATHER_KEY" ]]; then
        if [ -f "$(dirname "$0")/fetch_open_meteo.py" ]; then
            python3 "$(dirname "$0")/fetch_open_meteo.py" "${json_file}" "${UNIT}" >/dev/null 2>&1
            if [ -s "${json_file}" ] && ! grep -q '"desc": "No API Key"' "${json_file}" 2>/dev/null; then
                return
            fi
        fi
        write_dummy_data
