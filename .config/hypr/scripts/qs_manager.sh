...
if [[ "$ACTION" =~ ^[0-9]+$ ]]; then
    ...
    if [[ "$TARGET" == "move" ]]; then
        ...
        hyprctl dispatch movetoworkspace "$ACTION" >/dev/null 2>&1
    else
        hyprctl dispatch "hl.dsp.focus({ workspace = $ACTION })" >/dev/null 2>&1 || \
        hyprctl dispatch workspace "$ACTION" >/dev/null 2>&1
    fi
    exit 0
fi
...
handle_wallpaper_prep() {
    ...
    (
        ...
        cut -f1 "$MAP_FILE" | sort -u > "$SRC_LIST"
        ...
        rm -f "$SRC_LIST" "$MAP_FILE" "$PREP_LOCK"
